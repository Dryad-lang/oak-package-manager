...
fn test_async_keyword() {
    ...
    assert!(matches!(token, Token::Keyword(ref keyword) if keyword == "async"));
}
...
fn test_await_keyword() {
    ...
    let token = lexer.next_token().expect("Token esperado");
    assert!(matches!(token, Token::Keyword(ref keyword) if keyword == "await"));
}
...
fn test_thread_keyword() {
    ...
    let token = lexer.next_token().expect("Token esperado");
    assert!(matches!(token, Token::Keyword(ref keyword) if keyword == "thread"));
}
...
fn test_mutex_keyword() {
    ...
    let token = lexer.next_token().expect("Token esperado");
    assert!(matches!(token, Token::Keyword(ref keyword) if keyword == "mutex"));
}
...
fn test_join_keyword() {
    ...
    let token = lexer.next_token().expect("Token esperado");
    assert!(matches!(token, Token::Identifier(ref id) if id == "join"));
}
...
fn test_lock_keyword() {
    ...
    let token = lexer.next_token().expect("Token esperado");
    assert!(matches!(token, Token::Identifier(ref id) if id == "lock"));
}
...
fn test_unlock_keyword() {
    ...
    let token = lexer.next_token().expect("Token esperado");
    assert!(matches!(token, Token::Identifier(ref id) if id == "unlock"));
}
...
fn test_async_function_declaration() {
    ...
    let tokens: Vec<Token> = (0..6)
        .map(|_| lexer.next_token().expect("Token esperado"))
        .collect();
