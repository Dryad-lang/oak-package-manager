...
fn test_const_keyword_recognition() {
    ...
    assert_eq!(token, Token::Keyword("const".to_string()));
}
...
fn test_const_variable_declaration_lexing() {
    ...
    assert_eq!(token1, Token::Keyword("const".to_string()));
    
    // PI
    let token2 = lexer.next_token().unwrap();
    assert_eq!(token2, Token::Identifier("PI".to_string()));
    
    // =
    let token3 = lexer.next_token().unwrap();
    assert_eq!(token3, Token::Symbol('='));
    
    // 3.14159
    let token4 = lexer.next_token().unwrap();
    assert_eq!(token4, Token::Number(3.14159));
    
    // ;
    let token5 = lexer.next_token().unwrap();
    assert_eq!(token5, Token::Symbol(';'));
}
