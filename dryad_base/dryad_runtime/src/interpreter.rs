...
impl Value {
    ...
    fn is_truthy(&self) -> bool {
        match self {
            ...
            Value::AsyncFunction { .. } => true, // Funções async são truthy
            Value::ThreadFunction { .. } => true, // Funções thread são truthy
            Value::Lambda { .. } => true, // Lambdas são sempre truthy
            Value::Thread { is_running, .. } => *is_running, // Thread truthy se estiver rodando
            Value::Mutex { .. } => true, // Mutexes são sempre truthy
            Value::Promise { resolved, .. } => *resolved, // Promise truthy se resolvida
...
impl Interpreter {
    ...
    pub fn execute_statement(&mut self, stmt: &Stmt) -> Result<Value, DryadError> {
        match stmt {
            ...
            Stmt::ConstDeclaration(name, expr) => {
            ...
            Stmt::Assignment(name, expr) => {
                ...
                if self.constants.contains_key(name) {
                    return Err(DryadError::new(3011, &format!("Não é possível modificar a constante '{}'", name)));
                }
            ...
            Stmt::IndexAssignment(array_expr, index_expr, value_expr) => {
                let value = self.evaluate(value_expr)?;
            ...
            Stmt::AsyncFunctionDeclaration(name, params, body) => {
            ...
            Stmt::ClassDeclaration(name, parent, members) => {
                ...
                for member in members {
                    match member {
                        ...
                        ClassMember::AsyncMethod(visibility, is_static, method_name, params, body) => {
                            // Por enquanto, tratamos métodos async como métodos normais
            ...
            Stmt::Return(expr) => {
                ...
                match value {
                    ...
                    Value::AsyncFunction { .. } => Err(DryadError::new(3021, &format!("RETURN_OTHER:{}", value.to_string()))),
                    Value::ThreadFunction { .. } => Err(DryadError::new(3021, &format!("RETURN_OTHER:{}", value.to_string()))),
                    Value::Lambda { .. } => Err(DryadError::new(3021, &format!("RETURN_OTHER:{}", value.to_string()))),
                    Value::Thread { .. } => Err(DryadError::new(3021, &format!("RETURN_OTHER:{}", value.to_string()))),
                    Value::Mutex { .. } => Err(DryadError::new(3021, &format!("RETURN_OTHER:{}", value.to_string()))),
    ...
    pub fn evaluate(&mut self, expr: &Expr) -> Result<Value, DryadError> {
        match expr {
            ...
            Expr::ThreadCall(func_expr, args) => self.eval_thread_call(func_expr, args),
            Expr::MutexCreation => self.eval_mutex_creation(),
        }
    }
    ...
    fn eval_await(&mut self, expr: &Expr) -> Result<Value, DryadError> {
        // Por enquanto, await simplesmente executa a expressão de forma síncrona
        // TODO: Implementar execução assíncrona real com tokio
        match self.evaluate(expr)? {
            Value::Promise { id, resolved: true, value: Some(val) } => Ok(*val),
            Value::Promise { id, resolved: false, .. } => {
                // Simula espera por promise não resolvida
                Err(DryadError::new(4001, "Promise ainda não foi resolvida"))
            },
    ...
    fn execute_index_assignment(&mut self, array_expr: &Expr, index_value: Value, value: Value) -> Result<Value, DryadError> {
        // For now, we'll handle Variable expressions (like HashMap[key] = value)
        match array_expr {
            Expr::Variable(var_name) => {
                if let Some(array_val) = self.variables.get(var_name).cloned() {
                    match array_val {
                        Value::Array(mut elements) => {
                            ...
                            let new_array = Value::Array(elements);
                            self.variables.insert(var_name.clone(), new_array);
                            Ok(value)
                        },
                        Value::Object { mut properties, methods } => {
                            // Handle string key for objects (HashMap-like behavior)
                            let key = match index_value {
                                Value::String(s) => s,
                                Value::Number(n) => n.to_string(),
                                _ => return Err(DryadError::new(3084, "Chave do objeto deve ser string ou número")),
                            ...
                            let new_object = Value::Object { properties, methods };
                            self.variables.insert(var_name.clone(), new_object);
                            Ok(value)
                        },
                        _ => Err(DryadError::new(3085, "Tentativa de atribuir índice a valor que não é array nem objeto")),
                    }
                } else {
                    Err(DryadError::new(3001, &format!("Variável '{}' não foi declarada", var_name)))
                }
            },
            _ => Err(DryadError::new(3086, "Expressão de índice não suportada para atribuição")),
        }
    }
}
