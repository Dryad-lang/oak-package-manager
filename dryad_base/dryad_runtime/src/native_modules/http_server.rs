...
pub fn register_http_server_functions(functions: &mut HashMap<String, fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>>) {
    // Gerenciamento de servidor
    functions.insert("native_http_server_create".to_string(), native_http_server_create as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_start".to_string(), native_http_server_start as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_stop".to_string(), native_http_server_stop as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_status".to_string(), native_http_server_status as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    
    // Configuração de rotas
    functions.insert("native_http_server_route".to_string(), native_http_server_route as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_get".to_string(), native_http_server_get as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_post".to_string(), native_http_server_post as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_put".to_string(), native_http_server_put as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_delete".to_string(), native_http_server_delete as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    
    // Conteúdo estático
    functions.insert("native_http_server_static".to_string(), native_http_server_static as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_file".to_string(), native_http_server_file as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_html".to_string(), native_http_server_html as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_json".to_string(), native_http_server_json as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    
    // Configurações avançadas
    functions.insert("native_http_server_cors".to_string(), native_http_server_cors as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_http_server_middleware".to_string(), native_http_server_middleware as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
}
...
fn native_http_server_create(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
...
fn native_http_server_start(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    let server_id = match args.get(0) {
        Some(RuntimeValue::String(s)) => s.clone(),
...
fn native_http_server_stop(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    let server_id = match args.get(0) {
        Some(RuntimeValue::String(s)) => s.clone(),
        Some(_) => return Err(RuntimeError::TypeError("Argumento deve ser string (server_id)".to_string())),
...
fn native_http_server_status(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    let server_id = match args.get(0) {
        Some(RuntimeValue::String(s)) => s.clone(),
        Some(_) => return Err(RuntimeError::TypeError("Argumento deve ser string (server_id)".to_string())),
...
fn native_http_server_route(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    let server_id = match args.get(0) {
        Some(RuntimeValue::String(s)) => s.clone(),
...
/// Define uma rota GET
fn native_http_server_get(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() < 3 {
    ...
    native_http_server_route(&new_args)
}

/// native_http_server_post(server_id, path, response_body) -> null
/// Define uma rota POST
fn native_http_server_post(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() < 3 {
...
/// Define uma rota PUT
fn native_http_server_put(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() < 3 {
    ...
    let new_args = vec![
        args[0].clone(),
        RuntimeValue::String("PUT".to_string()),
        args[1].clone(),
        args[2].clone(),
    ];
...
fn native_http_server_delete(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() < 3 {
...
fn native_http_server_static(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 3 {
...
/// Alias para static
fn native_http_server_file(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    native_http_server_static(args)
}

/// native_http_server_html(server_id, path, html_content) -> null
/// Define resposta HTML
fn native_http_server_html(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 3 {
...
fn native_http_server_json(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 3 {
...
/// Configura CORS
fn native_http_server_cors(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
...
fn native_http_server_middleware(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
