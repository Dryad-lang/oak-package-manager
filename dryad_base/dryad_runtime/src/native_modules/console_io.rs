...
pub fn register_console_io_functions(functions: &mut std::collections::HashMap<String, fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>>) {
    ...
    functions.insert("print".to_string(), native_print);
...
/// native_input() - Lê linha do stdin (bloqueante)
/// Retorna: string
fn native_input(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    let stdin = io::stdin();
...
fn native_input_char(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
...
fn native_input_bytes(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_input_timeout(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_print(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
/// Args: data (qualquer tipo)
fn native_println(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_write_stdout(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
/// native_flush() - Força flush do stdout
fn native_flush(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    match stdout().flush() {
        Ok(_) => Ok(RuntimeValue::Null),
