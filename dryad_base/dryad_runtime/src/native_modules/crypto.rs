...
use rsa::signature::{SignatureEncoding, Verifier, Signer};
use std::sync::Mutex;

/// Registra todas as funções nativas do módulo crypto
pub fn register_crypto_functions(functions: &mut HashMap<String, fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>>) {
    functions.insert("sha256".to_string(), native_hash_sha256);
    functions.insert("native_hash_md5".to_string(), native_hash_md5 as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_uuid".to_string(), native_uuid as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_base64_encode".to_string(), native_base64_encode as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_base64_decode".to_string(), native_base64_decode as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_hex_encode".to_string(), native_hex_encode as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_hex_decode".to_string(), native_hex_decode as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_random_bytes".to_string(), native_random_bytes as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_random_string".to_string(), native_random_string as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_bytes_to_string".to_string(), native_bytes_to_string as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_encrypt_aes".to_string(), native_encrypt_aes as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_decrypt_aes".to_string(), native_decrypt_aes as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_encrypt_rsa".to_string(), native_encrypt_rsa as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_decrypt_rsa".to_string(), native_decrypt_rsa as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_sign".to_string(), native_sign as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_verify".to_string(), native_verify as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_generate_rsa_keypair".to_string(), native_generate_rsa_keypair as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
}
...
fn native_hash_sha256(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_hash_md5(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
/// native_uuid() -> string
fn native_uuid(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if !args.is_empty() {
...
fn native_base64_encode(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_base64_decode(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_hex_encode(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_hex_decode(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_random_bytes(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_random_string(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
...
fn native_bytes_to_string(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_encrypt_aes(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
...
fn native_decrypt_aes(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
...
fn native_encrypt_rsa(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
...
fn native_decrypt_rsa(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
...
fn native_sign(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
...
fn native_verify(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 3 {
...
fn native_generate_rsa_keypair(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
