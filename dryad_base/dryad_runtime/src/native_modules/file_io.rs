...
pub fn register_file_io_functions(functions: &mut HashMap<String, fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>>) {
    functions.insert("file_exists".to_string(), native_file_exists);
    functions.insert("native_read_file".to_string(), native_read_file as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_write_file".to_string(), native_write_file as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_append_file".to_string(), native_append_file as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_delete_file".to_string(), native_delete_file as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_list_dir".to_string(), native_list_dir as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_copy_file".to_string(), native_copy_file as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_move_file".to_string(), native_move_file as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_file_exists".to_string(), native_file_exists as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_is_dir".to_string(), native_is_dir as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_mkdir".to_string(), native_mkdir as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_getcwd".to_string(), native_getcwd as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_setcwd".to_string(), native_setcwd as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_get_file_info".to_string(), native_get_file_info as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
    functions.insert("native_read_file_content".to_string(), native_read_file_content as fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>);
}

/// Lê o conteúdo de um arquivo como uma string
/// Entrada: path (string)
/// Retorna: string com o conteúdo do arquivo
fn native_read_file(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
/// Retorna: null
fn native_write_file(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
        return Err(RuntimeError::ArgumentError(
...
/// Entrada: path (string), data (string)
/// Retorna: null
fn native_append_file(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
        return Err(RuntimeError::ArgumentError(
...
/// Entrada: path (string)
/// Retorna: null
fn native_delete_file(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
...
fn native_list_dir(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
...
/// Retorna: null
fn native_copy_file(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
        return Err(RuntimeError::ArgumentError(
...
/// Entrada: from (string), to (string)
/// Retorna: null
fn native_move_file(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 2 {
        return Err(RuntimeError::ArgumentError(
...
fn native_file_exists(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
...
fn native_is_dir(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
...
/// Entrada: path (string)
/// Retorna: null
fn native_mkdir(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
...
fn native_getcwd(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if !args.is_empty() {
        return Err(RuntimeError::ArgumentError(
...
/// Entrada: path (string)
/// Retorna: null
fn native_setcwd(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
...
fn native_get_file_info(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
...
fn native_read_file_content(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
        return Err(RuntimeError::ArgumentError(
