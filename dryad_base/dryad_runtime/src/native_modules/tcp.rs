...
pub fn register_tcp_functions(functions: &mut HashMap<String, fn(&[Value]) -> Result<Value, RuntimeError>>) {
    // Funções de servidor TCP
    functions.insert("tcp_server_create".to_string(), native_tcp_server_create as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_server_start".to_string(), native_tcp_server_start as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_server_stop".to_string(), native_tcp_server_stop as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_server_status".to_string(), native_tcp_server_status as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_server_set_max_clients".to_string(), native_tcp_server_set_max_clients as fn(&[Value]) -> Result<Value, RuntimeError>);
    
    // Funções de cliente TCP
    functions.insert("tcp_client_create".to_string(), native_tcp_client_create as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_client_connect".to_string(), native_tcp_client_connect as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_client_disconnect".to_string(), native_tcp_client_disconnect as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_client_send".to_string(), native_tcp_client_send as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_client_receive".to_string(), native_tcp_client_receive as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_client_status".to_string(), native_tcp_client_status as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_client_set_timeout".to_string(), native_tcp_client_set_timeout as fn(&[Value]) -> Result<Value, RuntimeError>);
    
    // Funções utilitárias
    functions.insert("tcp_resolve_hostname".to_string(), native_tcp_resolve_hostname as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_get_local_ip".to_string(), native_tcp_get_local_ip as fn(&[Value]) -> Result<Value, RuntimeError>);
    functions.insert("tcp_port_available".to_string(), native_tcp_port_available as fn(&[Value]) -> Result<Value, RuntimeError>);
}
...
fn native_tcp_server_create(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_server_start(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
/// Para o servidor TCP
fn native_tcp_server_stop(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_server_status(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_server_set_max_clients(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.len() < 2 {
...
fn native_tcp_client_create(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.len() < 3 {
...
fn native_tcp_client_connect(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_client_disconnect(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_client_send(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.len() < 2 {
...
fn native_tcp_client_receive(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_client_status(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_client_set_timeout(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.len() < 2 {
...
fn native_tcp_resolve_hostname(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
...
fn native_tcp_get_local_ip(_args: &[Value]) -> Result<Value, RuntimeError> {
...
fn native_tcp_port_available(args: &[Value]) -> Result<Value, RuntimeError> {
    if args.is_empty() {
