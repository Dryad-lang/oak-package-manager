...
pub fn register_time_functions(map: &mut HashMap<String, fn(&[RuntimeValue]) -> Result<RuntimeValue, RuntimeError>>) {
...
fn native_now(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
...
/// Retorna: nenhum
fn native_sleep(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
/// Entrada: nenhum
/// Retorna: um número inteiro representando o timestamp atual
fn native_timestamp(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    match SystemTime::now().duration_since(UNIX_EPOCH) {
...
fn native_date(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
...
fn native_time(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
...
fn native_format_date(args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
    if args.len() != 1 {
...
fn native_uptime(_args: &[RuntimeValue]) -> Result<RuntimeValue, RuntimeError> {
