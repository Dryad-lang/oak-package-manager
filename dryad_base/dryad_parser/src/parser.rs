...
impl Parser {
    ...
    pub fn statement(&mut self) -> Result<Option<Stmt>, DryadError> {
        match self.peek() {
            ...
            Token::Keyword(keyword) if keyword == "const" => {
                Ok(Some(self.const_declaration()?))
            }
    ...
    fn const_declaration(&mut self) -> Result<Stmt, DryadError> {
        ...
        let name = match self.advance() {
            Token::Identifier(name) => name.clone(),
            _ => return Err(DryadError::new(2012, "Esperado nome da constante após 'const'")),
        };
            ...
            Token::Keyword(k) if k == "thread" => {
                ...
                if matches!(self.peek(), Token::Symbol('(')) {
                    ...
                    if matches!(self.peek(), Token::Symbol(')')) {
                        ...
                        Err(DryadError::new(2031, "Esperado ')' após argumentos do thread"))
                    }
                } else {
    ...
    fn async_function_declaration(&mut self) -> Result<Stmt, DryadError> {
        ...
        // Expect opening parenthesis
        if !matches!(self.advance(), Token::Symbol('(')) {
            return Err(DryadError::new(2019, "Esperado '(' após nome da função async"));
        }
        ...
        if !matches!(self.peek(), Token::Symbol(')')) {
            loop {
                match self.advance() {
                    ...
                    _ => return Err(DryadError::new(2020, "Esperado nome do parâmetro na função async"))
                }
                ...
                match self.peek() {
                    ...
                    Token::Symbol(')') => break,
                    _ => return Err(DryadError::new(2021, "Esperado ',' ou ')' na lista de parâmetros da função async"))
                }
            }
        }
        ...
        // Expect closing parenthesis
        if !matches!(self.advance(), Token::Symbol(')')) {
            return Err(DryadError::new(2022, "Esperado ')' após parâmetros da função async"));
        }
        
        // Parse function body (block)
        let body = Box::new(self.block_statement()?);
        
        Ok(Stmt::AsyncFunctionDeclaration(name, params, body))
    }
    ...
    fn thread_function_declaration(&mut self) -> Result<Stmt, DryadError> {
        ...
        // Expect opening parenthesis
        if !matches!(self.advance(), Token::Symbol('(')) {
            return Err(DryadError::new(2025, "Esperado '(' após nome da função thread"));
        }
        ...
        if !matches!(self.peek(), Token::Symbol(')')) {
            loop {
                match self.advance() {
                    ...
                    _ => return Err(DryadError::new(2026, "Esperado nome do parâmetro na função thread"))
                }
                ...
                match self.peek() {
                    ...
                    Token::Symbol(')') => break,
                    _ => return Err(DryadError::new(2027, "Esperado ',' ou ')' na lista de parâmetros da função thread"))
                }
            }
        }
        ...
        // Expect closing parenthesis
        if !matches!(self.advance(), Token::Symbol(')')) {
            return Err(DryadError::new(2028, "Esperado ')' após parâmetros da função thread"));
        }
        
        // Parse function body (block)
        let body = Box::new(self.block_statement()?);
        
        Ok(Stmt::ThreadFunctionDeclaration(name, params, body))
    }
        ...
        match self.peek() {
            Token::Keyword(k) if k == "async" => {
                ...
                if matches!(next_token, Token::Keyword(kw) if kw == "function") {
                    ...
                    // Parse parameters
                    if !matches!(self.peek(), Token::Symbol('(')) {
                        return Err(DryadError::new(2092, "Esperado '(' após nome do método async"));
                    }
                    self.advance(); // consume '('
                    ...
                    Ok(ClassMember::AsyncMethod(visibility, is_static, name, params, body))
                } else {
                    return Err(DryadError::new(2096, "Esperado 'function' após 'async' em classe"));
                }
            }
            Token::Keyword(k) if k == "function" => {
                self.advance(); // consume 'function'
    ...
    fn parse_object_property(&mut self) -> Result<crate::ast::ObjectProperty, DryadError> {
        ...
        let key = match self.advance() {
            ...
            _ => return Err(DryadError::new(2072, "Esperado identificador ou string como chave da propriedade")),
        };
    ...
    fn index_assignment_statement(&mut self) -> Result<Stmt, DryadError> {
        ...
        let array_expr = match self.peek() {
            ...
            _ => return Err(DryadError::new(2101, "Esperado identificador para index assignment")),
        };
        ...
        Ok(Stmt::IndexAssignment(array_expr, index_expr, value_expr))
    }
