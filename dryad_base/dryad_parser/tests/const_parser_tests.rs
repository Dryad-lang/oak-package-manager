...
fn test_parse_simple_const_declaration() {
    ...
    match &program.statements[0] {
        Stmt::ConstDeclaration(name, expr) => {
            assert_eq!(name, "PI");
            match expr {
                Expr::Literal(Literal::Number(n)) => assert_eq!(*n, 3.14159),
...
fn test_parse_const_string_declaration() {
    ...
    match &program.statements[0] {
        Stmt::ConstDeclaration(name, expr) => {
            ...
            match expr {
                Expr::Literal(Literal::String(s)) => assert_eq!(s, "Dryad Language"),
...
fn test_multiple_const_declarations() {
    ...
    match &program.statements[0] {
        Stmt::ConstDeclaration(name, _) => assert_eq!(name, "PI"),
        _ => panic!("Esperado ConstDeclaration"),
    }
    
    // Verifica segunda constante
    match &program.statements[1] {
        Stmt::ConstDeclaration(name, _) => assert_eq!(name, "E"),
        _ => panic!("Esperado ConstDeclaration"),
    }
    
    // Verifica terceira constante
    match &program.statements[2] {
        Stmt::ConstDeclaration(name, _) => assert_eq!(name, "NAME"),
        _ => panic!("Esperado ConstDeclaration"),
    }
}